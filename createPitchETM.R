...
createPitch <- function(xmax=115, ymax=80, grass_colour="white", line_colour="gray", background_colour="white", goal_colour="gray",spot.size=.75, data=NULL, halfPitch=FALSE){
  ...
  if (halfPitch==FALSE){
    ...
    ggplot(data=data) + #xlim(c(ymin,ymax)) + ylim(c(xmin,xmax)) +
      ...
      geom_point(aes(x = penspot , y = ymax/2), colour = line_colour, size=spot.size) + 
      # add penalty spot right
      geom_point(aes(x = (xmax-(penspot)) , y = ymax/2), colour = line_colour,  size=spot.size) + 
      # add centre spot 
      geom_point(aes(x = (xmax/2) , y = ymax/2), colour = line_colour, size=spot.size) +
